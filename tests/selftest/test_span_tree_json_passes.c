...
**       PASSES variant — serializer and parser are correct, no bug.
**       See test_span_tree_json_shrinks.c for the deliberately-buggy
**       variant that hegel detects and shrinks.
**
...
**
** Layer 1: tree_to_json + json_to_tree (a tiny correct serializer/parser).
** Layer 2: hegel test that round-trips a random tree.
** Layer 3: main runner.
**
** Expected: EXIT 0.  The serializer/parser are written to match each
** other exactly, so the round-trip property holds for every input the
** generator produces.
*/
#include <stdio.h>
#include <stdlib.h>
